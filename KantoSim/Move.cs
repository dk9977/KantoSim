...
namespace KantoSim
{
    public abstract class Move
    {
        ...
        private static bool SecondaryHealHalfDamageDealt(ushort damage, Battler user)
        {
            user.Heal((ushort)(damage < 2 ? 1 : damage / 2));
            return true;
        }
        private static bool SecondaryAcid(Battler target)
        {
            if (rng.Next(3) == 0)
                return target.ModifyStat(Battler.StatMod.Def, -1);
            return false;
        }
        private static bool SecondaryAcidArmor(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Def, 2);
        }
        private static bool SecondaryAgility(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Spe, 2);
        }
        private static bool SecondaryAmnesia(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Spc, 2);
        }
        private static bool SecondaryAuroraBeam(Battler target)
        {
            if (rng.Next(3) == 0)
                return target.ModifyStat(Battler.StatMod.Atk, -1);
            return false;
        }
        private static bool SecondaryMultiHit(ushort damage, Battler target)
        {
            int r = rng.Next(0b1000);
            if (r == 0)
            {
        ...
        private static bool SecondaryBarrier(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Def, 2);
        }
        private static bool SecondaryBide(Battler user)
        {
            // apply Biding volatile status to user
            return true;
        }
        private static bool SecondaryBind(Battler user, Battler target)
        {
            // apply Binding volatile status to user
            // apply Bound volatile status to target
            return true;
        }
        private static bool SecondaryBite(Battler target)
        {
            if (rng.Next(10) == 0)
            {
                // apply Flinching volatile status to target
                return true;
            }
            return false;
        }
        private static bool SecondaryBlizzard(Battler target)
        {
            if (rng.Next(10) == 0)
            {
        ...
        private static bool SecondaryBodySlam(Battler target)
        {
            if (rng.Next(10) < 3)
            {
        ...
        private static bool SecondaryBoneClub(Battler target)
        {
            if (rng.Next(10) == 0)
            {
                // apply Flinching volatile status to target
        ...
        private static bool SecondaryDoubleHit(ushort damage, Battler target)
        {
            target.Damage(damage);
            return true;
        }
        private static bool SecondarySlow(Battler target)
        {
            if (rng.Next(3) == 0)
                return target.ModifyStat(Battler.StatMod.Spe, -1);
            return false;
        }
        private static bool SecondaryConfuseRay(Battler target)
        {
            // apply Confusion volatile status to target
            return true;
        }
        private static bool SecondaryConfusion(Battler target)
        {
            if (rng.Next(10) == 0)
            {
                // apply Confusion volatile status to target
        ...
        private static bool SecondaryConversion(Battler user)
        {
            // change type of user
            return true;
        }
        private static bool SecondaryCounter(ushort damage, Battler target)
        {
            target.Damage((ushort)(damage * 2));
            return true;
        }
        private static bool SecondaryDefenseCurl(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Def, 1);
        }
        private static bool SecondaryDig(Battler user)
        {
            // apply Charging and Underground volatile statuses to user
            return true;
        }
        private static bool SecondaryDisable(Battler target)
        {
            // apply Disabled volatile status and DisabledMove index to target
            return true;
        }
        private static bool SecondaryDoubleTeam(Battler user)
        {
            return user.ModifyStat(Battler.StatMod.Eva, 1);
        }
        private static bool SecondaryDoubleEdge(ushort x, Battler user)
        {
            return user.Damage((ushort)(x / 4));
        }
        private static bool SecondaryDragonRage(Battler target)
        {
            target.Damage(40);
            return true;
        }
        private static bool SecondaryMinorBurnChance(Battler target)
        {
            if (rng.Next(10) == 0)
            {
                // apply Burn non-volatile status to target
                return true;
            }
            return false;
        }
        private static bool SecondaryExplosion(Battler user)
        {
            user.Damage(ushort.MaxValue);
            return true;
        }

        public readonly Move[] movedex = {
            new Move("Absorb", Type.Grass, true, 20, 100, 20, (x, a, d) => SecondaryHealHalfDamageDealt(x, a)),
            new Move("Acid", Type.Poison, true, 40, 100, 30, (x, a, d) => SecondaryAcid(d)),
            new Move("Acid Armor", Type.Poison, false, 0, 0, 40, (x, a, d) => SecondaryAcidArmor(a)),
            new Move("Agiliy", Type.Psychic, false, 0, 0, 30, (x, a, d) => SecondaryAgility(a)),
            new Move("Amnesia", Type.Psychic, false, 0, 0, 20, (x, a, d) => SecondaryAmnesia(a)),
            new Move("Aurora Beam", Type.Ice, true, 65, 100, 20, (x, a, d) => SecondaryAuroraBeam(d)),
            new Move("Barrage", Type.Normal, true, 15, 85, 20, (x, a, d) => SecondaryMultiHit(x, d)),
            new Move("Barrier", Type.Psychic, false, 0, 0, 30, (x, a, d) => SecondaryBarrier(a)),
            new Move("Bide", Type.Normal, true, 0, 0, 10, (x, a, d) => SecondaryBide(a)),
            new Move("Bind", Type.Normal, true, 15, 75, 20, (x, a, d) => SecondaryBind(a, d)),
            new Move("Bite", Type.Normal, true, 60, 100, 25, (x, a, d) => SecondaryBite(d)),
            new Move("Blizzard", Type.Ice, true, 120, 90, 5, (x, a, d) => SecondaryBlizzard(d)),
            new Move("Body Slam", Type.Normal, true, 85, 100, 15, (x, a, d) => SecondaryBodySlam(d)),
            new Move("Bone Club", Type.Normal, true, 65, 85, 20, (x, a, d) => SecondaryBoneClub(d)),
            new Move("Bonemerang", Type.Normal, true, 50, 90, 10, (x, a, d) => SecondaryDoubleHit(x, d)),
            new Move("Bubble", Type.Water, true, 20, 100, 30, (x, a, d) => SecondarySlow(d)),
            new Move("Bubble Beam", Type.Water, true, 65, 100, 20, (x, a, d) => SecondarySlow(d)),
            new Move("Clamp", Type.Water, true, 35, 75, 10, (x, a, d) => SecondaryBind(a, d)),
            new Move("Comet Punch", Type.Normal, true, 18, 85, 15, (x, a, d) => SecondaryMultiHit(x, d)),
            new Move("Confuse Ray", Type.Ghost, false, 0, 100, 10, (x, a, d) => SecondaryConfuseRay(d)),
            new Move("Confusion", Type.Psychic, true, 50, 100, 25, (x, a, d) => SecondaryConfusion(d)),
            new Move("Constrict", Type.Normal, true, 10, 100, 35, (x, a, d) => SecondarySlow(d)),
            new Move("Conversion", Type.Normal, false, 0, 0, 30, (x, a, d) => SecondaryConversion(a)),
            new Move("Counter", Type.Fighting, true, 0, 100, 20, (x, a, d) => SecondaryCounter(x, d)),
            new Move("Crabhammer", Type.Water, true, 90, 85, 10, null),
            new Move("Cut", Type.Normal, true, 50, 95, 30, null),
            new Move("Defense Curl", Type.Normal, false, 0, 0, 40, (x, a, d) => SecondaryDefenseCurl(a)),
            new Move("Dig", Type.Ground, true, 100, 100, 10, (x, a, d) => SecondaryDig(a)),
            new Move("Disable", Type.Normal, false, 0, 55, 20, (x, a, d) => SecondaryDisable(d)),
            new Move("Dizzy Punch", Type.Normal, true, 70, 100, 10, null),
            new Move("Double Kick", Type.Fighting, true, 30, 100, 30, (x, a, d) => SecondaryDoubleHit(x, d)),
            new Move("Double Slap", Type.Normal, true, 15, 85, 10, (x, a, d) => SecondaryMultiHit(x, d)),
            new Move("Double Team", Type.Normal, false, 0, 0, 15, (x, a, d) => SecondaryDoubleTeam(a)),
            new Move("Double-Edge", Type.Normal, true, 100, 100, 15, (x, a, d) => SecondaryDoubleEdge(x, a)),
            new Move("Dragon Rage", Type.Dragon, true, 0, 100, 10, (x, a, d) => SecondaryDragonRage(d)),
            new Move("Dream Eater", Type.Psychic, true, 100, 100, 15, (x, a, d) => SecondaryHealHalfDamageDealt(x, a)),
            new Move("Drill Peck", Type.Flying, true, 80, 100, 20, null),
            new Move("Earthquake", Type.Ground, true, 100, 100, 10, null),
            new Move("Egg Bomb", Type.Normal, true, 100, 75, 10, null),
            new Move("Ember", Type.Fire, true, 40, 100, 25, (x, a, d) => SecondaryMinorBurnChance(d)),
            new Move("Explosion", Type.Normal, true, 170, 100, 5, (x, a, d) => SecondaryExplosion(a)),
        };
    }
